...
module.exports = (methods, routes, auth, events, plugins, options) => {
  ...
  const menu = '<div style="float: right"><a href="#routes">Routes</a> | <a href="#methods">Methods</a> | <a href="#events">Events</a> | <a href="#strategies">Strategies</a> | <a href="#plugins">Plugins</a></div>';
  const html = `
    <style>
      table {
        width: 100%;
    ...
    <table>
      ...
      ${Object.keys(events).map(r => `
        <tr>
        <td>${r}: ${events[r].handlers}</td>
        <td>${events[r].description || ''}</td>
      `).join('')}
    </table>
    ${menu}
