...
module.exports = function(server, options, meta) {
  const unsortedRoutes = server.table().reduce((memo, route) => {
    ...
    if (meta.routes && meta.routes[routeInfo.path]) {
      Object.assign(routeInfo, meta.routes[routeInfo.path]);
    }
    memo.push(routeInfo);
    return memo;
  }, []);
  // routes will be sorted alphabetically
  return unsortedRoutes.sort((a, b) =>
    a.path.toLowerCase().toLowerCase().localeCompare(b.path.toLowerCase()));
};
